...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>Phuc Hub "beta" chào ae<Color=/>"):Display()
    wait(.5)
...
local Window = OrionLib:MakeWindow({Name = "Phuc Hub "beta"", HidePremium = false, IntroText = "Phuc Hub "beta"⚜️", SaveConfig = true, ConfigFolder = "NightHub"})
...
W:AddLabel("Phuc Hub "beta" SCRIPT")
W:AddLabel("DiscordPhuc Hub "beta"")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "Phuc Hub "beta"(Noitification)",
